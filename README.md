# Hello, my name is Alejandro Espínola 👋
### Computer Science student at [![FP-UNA](https://img.shields.io/badge/FP_UNA-338BD1)](https://www.pol.una.py)

I'm a 21-year-old Software Developer and Computer Science student at FP-UNA, specializing in Enterprise Software, backend architecture, and modern Fullstack solutions.

Currently, I work at [Interfaces S.A.](https://www.interfaces.com.py/) — the leading Odoo Gold Partner in Paraguay. Over the past year, I have been developing complex ERP business logic, optimizing relational databases, and engineering custom enterprise features (CRM, WMS, Accounting, and localized billing systems) using Python and Odoo.

### 🚀 Technical Focus & Evolution:
* **Enterprise Software & Backend Logic:** Advanced workflows in Odoo ERP, relational database engineering (PostgreSQL), and process automation for large-scale corporations.
* **Fullstack Architecture:** Bridging robust corporate backends with modern user experiences using **Ruby on Rails** (API-centric) and **React** for building clean, dynamic, and high-performance frontends.
* **Modern Tooling:** Deepening skills in version control (Git) and optimizing development environments with **Docker** for clean workflows.

---

### 🛠️ Tech Stack & Tools

#### Backend & Databases
[![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=Python&logoColor=3776AB&labelColor=101010)]()
[![Odoo](https://img.shields.io/badge/Odoo-714B67?style=for-the-badge&logo=odoo&logoColor=white&labelColor=101010)]()
[![Ruby on Rails](https://img.shields.io/badge/Ruby_on_Rails-CC0000?style=for-the-badge&logo=ruby-on-rails&logoColor=white&labelColor=101010)]()
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white&labelColor=101010)]()

#### Frontend
[![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB&labelColor=101010)]()
[![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black&labelColor=101010)]()
[![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white&labelColor=101010)]()
[![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white&labelColor=101010)]()

#### Tools & DevOps
[![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white&labelColor=101010)]()
[![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white&labelColor=101010)]()

---

### 📈 Current Goals
* Finish my interactive fullstack portfolio using Rails + React.
* Keep creating the perfect balance between heavy enterprise business logic and modern, fluid user experiences.
